...
const TRIGGER: &[(Scopes, &str, Trigger)] = &[
    ...
    (Scopes::Character, "can_inherit", Boolean),
    (Scopes::Character, "can_marry", Boolean),
    ...
    (Scopes::Country, "has_claim", ScopeOrItem(Scopes::Province, Item::Province)),
    ...
    (Scopes::Country, "is_trade_goods_allowed", Item(Item::TradeGood)),
    ...
    (Scopes::Province, "is_holy_site_of_deity", Scope(Scopes::Deity)),
    (Scopes::Province, "is_holy_site_of_religion", ScopeOrItem(Scopes::Religion, Item::Religion)),
...
pub fn trigger_schema(name: &str) -> Option<Vec<crate::lsp_tables::SchemaField>> {
    ...
    for (_, s, trigger) in TRIGGER.iter() {
        ...
        if let Trigger::Block(f) = trigger {
